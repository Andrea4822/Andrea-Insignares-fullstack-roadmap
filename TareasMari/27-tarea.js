...
function create(bookCollection) {
  let totalPagesforgenr = {}

  for (let i = 0; i < bookCollection.length; i++) {
    for (let j = 0; j < bookCollection[i].genre.length; j++) {
        let paginas = bookCollection[i].pages;
        let generos = bookCollection[i].genre[j];

        if (totalPagesforgenr[generos]){
            totalPagesforgenr[generos] += paginas;
        }else{
            totalPagesforgenr[generos] = paginas;
        }
    }
  ...
  return totalPagesforgenr;
 
}

console.table(create(bookCollection));

//const resumenDePaginasPorGenero = create(bookCollection);

//console.table (resumenDePaginasPorGenero)
