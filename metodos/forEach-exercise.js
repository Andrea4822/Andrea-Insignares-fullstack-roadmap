...
/*function logBookSummaries(bookCollection){

  let result = [];
  bookCollection.forEach(a => {
    let summary= ` ${a.title} by ${a.author} (${a.publicationYear})`;
    result.push(summary);
...
console.log (logBookSummaries(bookCollection));*/
...
/*function countAvailableBooks(bookCollection){
  let count=0;
...
/*function getSciFiBooks(bookCollection){
let array= [];
bookCollection.forEach(book => {
if (book.genre.includes("Science Fiction")){
  let filter = `${book.title}, ${book.publicationYear},${book.author}`
  array.push(filter);
}
});
return array;
...
/*function findHighestRatedBook(bookCollection){
  let betterRating= 0;
  let betterbook= " ";
  bookCollection.forEach(book => {
    book.ratings.forEach(rating=> {
    if (rating.source.includes("Goodreads")){
      if (rating.score >betterRating){
        betterRating= rating.score;
        betterbook= book.title;
      }
    } 
...
/*function getHighRatedBooks (bookCollection){
  ...
  bookCollection.forEach(book => {
    book.forEach(rating => {
      let result= book.some(rating => rating.ratings.find(rating => rating.source==="Goodreads").score> 4.8);
      result= 
      array.push(book[i].title);
