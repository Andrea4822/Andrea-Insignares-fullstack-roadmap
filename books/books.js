...
function getBooksByAvailability(isAvailability) {
  ...
  for (let i = 0; i <bookCollection.length; i++) {
    if (bookCollection[i].isAvailable === isAvailability) {
      arrayResult.push(bookCollection[i]);
    }
  }
  return arrayResult;
}
...
function getBooksForYearsRange (añoInicial, añoFinal){
    let arrayResult2 =[]
    for (let i = 0; i <bookCollection.length; i++) {
        if (bookCollection[i]. publicationYear === añoInicial){
            arrayResult2.push(bookCollection[i]);
        }
...
console.log (getBooksForYearsRange(true));