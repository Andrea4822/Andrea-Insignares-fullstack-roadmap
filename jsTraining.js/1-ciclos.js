...
const saludo = "hola muchachos, hoy vamos a ver ciclos en javascript"; //declara variable externa

let saludoParseado = ""; // declara variable extrena como contenedor 

for (let index = 0; index < saludo.length; index++) { // recorre todo el saludo de la variable const externa
    if (saludo[index] === " ") { // crea condicion y dice sí saldo en la posición [i] es igual a un espacio
        saludoParseado += "_" // se agrega un guion bajo
    } else {
        saludoParseado += saludo[index] // si la condicion anterior no se cumple entonces devuelve el saludo normal
    }
}

console.log(saludoParseado); // imprime el saludo
